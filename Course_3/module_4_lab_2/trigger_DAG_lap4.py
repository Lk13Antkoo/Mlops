...
with DAG (
    ...
    if not is_venv_installed():
        ...
        @task.virtualenv(
            task_id = 'store_database', requirements = ['pandas==2.1.1','numpy>=1.21,<1.25' ,'sqlalchemy==2.0.21'], system_site_packages=False
        )
        def persit_dataset():
            import pandas as pd
            from sqlalchemy import create_engine
            engine = create_engine('sqlite://///home/project/AZURE_DUKE/course_3/temp/wine_databse.db',echo=True)
            df = pd.read_csv("/home/project/AZURE_DUKE/course_3/module_2/lab_2_Create_dag_pandas/wine_fine_funed.csv")
            df.to_sql('wine_databse', engine)
            df.notes.to_sql("wine_notes", engine)

        pandas_write() >> persit_dataset()
